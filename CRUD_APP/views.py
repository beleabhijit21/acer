...
def show_view(request):
    ...
    templates_name = 'CRUD_APP/show.html'
    context = {'data':obj}
    return render(request,templates_name,context)

def delete_view(request, pk):
    obj = Order.objects.filter(oid=pk)
    if request.method == 'POST':
        for i in obj:
            i.delete()
        return redirect('show_url')
    ...
    #obj.delete()
   # return redirect('show_url')
   # return HttpResponse(f'<h1>ID is:{pk}</h1>')
   


def update_view(request, upk):
    print(upk)
    obj = Order.objects.get(oid=upk)
    ...
    context = {'form': form}
    return render(request,template_name,context)
  #  return HttpResponse(f'<h1>ID is:{upk}</h1>')